 ...
 * 
 * @author Rayliu40k
 * @version $Id: Wc5_加餐.java, v 0.1 Jul 26, 2020 4:00:58 PM Rayliu40k Exp $
 */
public class Wc5_加餐 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.print("请输入起始年数:");
        String inputstart = br.readLine();
        System.out.print("请输入结束年数:");
        String inputend = br.readLine();
        int start = Integer.valueOf(inputstart);
        int end = Integer.valueOf(inputend);
        if (start < 1930 && end > 2018) {
            System.out.println("[WARNING]不能小于1930或大于2018!");
            main(args);
        }
        if (start > end) {
            System.out.println("[WARNING]结束年数不能小于起始年数!");
            main(args);
        }
        Set<Entry<Integer, Wc_cs>> entrys = wc.entrySet();
        for (Entry<Integer, Wc_cs> entry : entrys) {
            if (entry.getValue().getJvBanNianShu() >= start
                && entry.getValue().getJvBanNianShu() <= end) {
                System.out
